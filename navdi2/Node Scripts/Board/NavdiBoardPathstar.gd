...
func setup_from_tilemap(tilemap, solid_flag_ids = DefaultSolidFlagIds):
	self.clear()
...
#		print("has func? ",tilemap.has_method("get_cellv_flag"))
		if tilemap.get_cellv_flag(cell) in solid_flag_ids:
...
func zonegrid_point_to_cell(point):
	if point != null: return Vector2(
		point % int(_scheme_zonegrid_zone.size.x), + 
