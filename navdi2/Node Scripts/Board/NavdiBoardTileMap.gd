...
func get_cellv_flag(cell):
	var cellvalue = get_cellv(cell)
	if cellvalue <= 0: return Flag.ILLEGALTile
	if cellvalue >= len(tile_set.values_to_flags): return Flag.ILLEGALTile
	return tile_set.values_to_flags[cellvalue]
